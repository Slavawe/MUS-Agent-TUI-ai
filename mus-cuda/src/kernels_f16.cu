...
// ══════════════════════════════════════════════════════════════════════
//  AdamW (FP32, same as before — optimizer stays FP32)
// ══════════════════════════════════════════════════════════════════════
...
__global__ void rmsnorm_bwd_kernel_f16(const half* __restrict__ x,
    ...
    float ir = 1.0f / rms;
    float ldw = 0.0f;
    for (int i = tid; i < cols; i += bs) ldw += __half2float(rdy[i]) * __half2float(rx[i]) * ir;
    ldw = warp_reduce_sum_f16(ldw);
    if ((tid % 32) == 0) atomicAdd(dw + tid / 32, ldw);
    for (int i = tid; i < cols; i += bs) rdx[i] = __float2half(__half2float(rdy[i]) * w[i] * ir);
}
...
void mus_attention_backward_f16(MUSContext* ctx,
    ...
    mus_gemm_f16(ctx, CUBLAS_OP_T, CUBLAS_OP_N, D,rows,D, d_qkv_w, 3*D, dQ_flat,D, d_x,D);
    mus_gemm_f16(ctx, CUBLAS_OP_T, CUBLAS_OP_N, D,rows,D, d_qkv_w+D, 3*D, dK_flat,D, d_x,D);
    mus_gemm_f16(ctx, CUBLAS_OP_T, CUBLAS_OP_N, D,rows,D, d_qkv_w+2*D, 3*D, dV_flat,D, d_x,D);
    mus_gemm_f16(ctx, CUBLAS_OP_T, CUBLAS_OP_N, D,rows,D, o_w,D, d_out,D, temp_h,D);
    int n = rows * D;
...
void mus_transformer_block_forward_f16(MUSContext* ctx,
    ...
    size_t need = 4 * sz;
    if (ctx->workspace_size < need + 200*1024*1024) { fprintf(stderr, "[block_fwd_f16] ws\n"); return; }
